...
import com.bcaf.finapay.services.LoanRequestService;
...
public class LoanRequestController {
    ...
    @GetMapping("/by-email")
    public ResponseEntity<ResponseDto> getAllLoanRequestsByEmail(@RequestParam(required = false) String status) {
        LoanStatus loanStatus = null;
        if (status != null && !status.isEmpty()) {
            try {
                loanStatus = LoanStatus.valueOf(status.toUpperCase());
            } catch (IllegalArgumentException e) {
                throw new IllegalArgumentException("Status tidak ditemukan");
            }
        }
        List<?> loanRequests = loanRequestService.getAllLoanRequestsByEmail(loanStatus);
        return ResponseEntity
                .ok(new ResponseDto(200, "success", loanRequests.size() + " loan requests found", loanRequests));
    }
